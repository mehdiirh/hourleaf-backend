...
def date_range(params):
    ...
    try:
        start = date.fromisoformat(
            ...
            params.get("to", date.today().replace(month=12, day=31).isoformat())
        )
    except (ValueError, TypeError):
        raise ValidationError("Dates must use YYYY-MM-DD.")
    if end < start or (end - start).days > 365:
        raise ValidationError(
            "Choose a range of at most 366 days, with the end after the start."
        )
    return start, end
    ...
    def get_queryset(self):
        ...
        if self.action == "list":
            ...
            if self.request.query_params.get("work_type"):
                qs = qs.filter(
                    work_type__normalized_name=normalize_type(
                        self.request.query_params["work_type"]
                    )
                )
        return qs
    ...
    def _save(self, serializer):
        with transaction.atomic():
            ...
            minutes = serializer.validated_data.get(
                "minutes", instance.minutes if instance else 0
            )
            ...
            if total + minutes > 1440:
                raise ValidationError(
                    "A day cannot contain more than 24 hours of work."
                )
            ...
            if (
                start
                and end
                and others.filter(start_time__lt=end, end_time__gt=start).exists()
            ):
                raise ValidationError(
                    "This time range overlaps another entry on this date."
                )
            ...
            if name is not None:
                work_type, _ = WorkType.objects.get_or_create(
                    ...
                    normalized_name=normalize_type(name),
                    defaults={"name": name},
                )
...
class WorkTypesView(APIView):
    def get(self, request):
        ...
        return Response(
            list(
                qs.annotate(uses=Count("entries"), last_used=Max("entries__date"))
                .order_by("-uses", "name")
                .values("id", "name", "uses", "last_used")[:100]
            )
        )
...
class AnalyticsView(APIView):
    def get(self, request):
        ...
        for day in days:
            run = (
                run + 1
                if previous and day["date"] == previous + timedelta(days=1)
                else 1
            )
            best = max(best, run)
            previous = day["date"]
        return Response(
            {
                ...
                "average_minutes": round(total / len(days)) if days else 0,
                "longest_streak": best,
                "days": days,
                "categories": [
                    {
                        "name": c["work_type__name"],
                        "minutes": c["minutes"],
                        "entries": c["entries"],
                    }
                    for c in categories
