...
class EntrySerializer(serializers.ModelSerializer):
    ...
    start_time = serializers.TimeField(required=False, allow_null=True, input_formats=["%H:%M"], format="%H:%M")
    end_time = serializers.TimeField(required=False, allow_null=True, input_formats=["%H:%M"], format="%H:%M")
    class Meta:
        model = Entry
        fields = ["id", "date", "work_type", "duration", "minutes", "start_time", "end_time", "created_at"]
    ...
    def validate(self, attrs):
        ...
        if duration is not None:
            ...
            if not re.fullmatch(r"\d{1,2}:[0-5]\d", duration):
                raise serializers.ValidationError({"duration": "Use HH:MM, for example 07:30."})
            hours, mins = map(int, duration.split(":"))
            attrs.update(minutes=hours * 60 + mins, start_time=None, end_time=None)
        elif has_times:
            if start is None or end is None:
                raise serializers.ValidationError("Both start and end times are required.")
            minutes = (end.hour * 60 + end.minute) - (start.hour * 60 + start.minute)
            if minutes <= 0:
                raise serializers.ValidationError("End time must be after start time. Split overnight work into two dates.")
        ...
        if "minutes" in attrs and not 1 <= attrs["minutes"] <= 1440:
            raise serializers.ValidationError({"duration": "Duration must be between 00:01 and 24:00."})
        return attrs
